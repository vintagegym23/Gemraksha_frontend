...
import { ShoppingBag, Star, ShieldCheck, Video, Truck, RefreshCw, ChevronRight, Sparkles } from 'lucide-react';
...
const BraceletDetail = ({ productSlug }: { productSlug: string }) => {
    ...
    const fetchProduct = async () => {
      ...
      try {
        ...
        if (data) {
          ...
          // Fetch related bracelets
          const allProducts = await api.getProducts();
          setRelatedProducts(allProducts.filter(p => p.id !== data.id && (p.category === 'Healing Bracelets' || p.category === 'Luxury Bracelets')).slice(0, 4));
        } else {
          navigate('/bracelets');
        }
      } catch (error) {
    ...
    <div className="min-h-screen bg-[#fbf9f6] pb-28 md:bg-white md:pb-16 pt-4 md:pt-8 w-full">
      <div className="mx-auto max-w-[1240px] px-4 md:px-8">
        ...
        <div className="hidden md:flex items-center text-[10px] uppercase tracking-wider text-[#888] mb-8 gap-2">
          ...
          <Link to="/bracelets" className="hover:text-[#4a154b]">Bracelets</Link>
          <ChevronRight size={10} />
        ...
        <div className="flex flex-col md:flex-row gap-0 md:gap-16">
          ...
          <div className="w-full md:w-[50%] flex flex-col-reverse md:flex-row gap-4 mb-6 md:mb-0">
            ...
            <div className="relative flex-1 aspect-square md:aspect-auto md:h-[600px] bg-[#f9f9f9] md:bg-transparent rounded-none md:rounded-xl overflow-hidden flex items-center justify-center">
              ...
              <div className="absolute top-4 left-4 flex items-center gap-1.5 rounded-sm bg-[#d0a061] px-3 py-1 scale-90 origin-top-left md:scale-100">
                <span className="text-[10px] font-bold uppercase tracking-[0.1em] text-white">Healing Bracelet</span>
              </div>
            </div>
          </div>
          ...
          <div className="w-full md:w-[45%] flex flex-col px-4 md:px-0 mt-4 md:mt-0">
            ...
            <div className="flex items-center gap-2 mb-6">
              ...
              <span className="text-[11px] text-[#888]">(28 Reviews) • <span className="text-[#22c55e]">In Stock</span></span>
            </div>
            ...
            <div className="mb-8 bg-[#fcfaf8] rounded-lg p-4 border border-[#f0ebd9]">
              <h3 className="text-[12px] font-bold uppercase tracking-wider text-[#555] mb-4">Bracelet Details</h3>
              <div className="grid grid-cols-2 gap-4 text-[12px]">
                ...
                {product.details.shape && (
                  <div>
                    <span className="text-[#888] block mb-1">Bead Type</span>
                    <span className="font-medium text-[#251622]">{product.details.shape}</span>
                  </div>
                )}
                {product.details.treatment && (
                  <div>
                    <span className="text-[#888] block mb-1">Treatment</span>
                    <span className="font-medium text-[#251622]">{product.details.treatment}</span>
                  </div>
                )}
            ...
            <div className="mb-8">
              <div className="flex items-center gap-2 mb-4">
                <Sparkles size={16} className="text-[#d0a061]" />
                <span className="text-[11px] font-bold uppercase tracking-wider text-[#555]">Healing Properties</span>
              </div>
              <p className="text-[12px] leading-relaxed text-[#666]">
                This bracelet is believed to promote physical wellness, emotional balance, and spiritual growth. Wear it daily to harness the natural energy of these precious gemstones.
              </p>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 border-t border-b border-gray-100 py-6 mb-4">
              <div className="flex flex-col items-center justify-center text-center gap-2">
                ...
                <span className="text-[9px] uppercase tracking-wider text-[#666] font-bold">Authentic Stones</span>
              </div>
              <div className="flex flex-col items-center justify-center text-center gap-2">
        ...
        <div className="mt-12 md:mt-20 border-t border-gray-200 pt-8 pb-12 px-4 md:px-0">
          <div className="flex overflow-x-auto no-scrollbar gap-8 border-b border-gray-200 mb-8">
            {['Healing Properties', 'How to Use', 'Care & Cleansing'].map((tab) => (
              <button
                key={tab}
          ...
          <div className="min-h-[200px]">
            {activeTab === 'Healing Properties' && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col gap-10">
                <div className="max-w-3xl">
                  <h3 className="text-xl font-serif text-[#251622] mb-4">Spiritual & Healing Benefits</h3>
                  <p className="text-[13px] leading-relaxed text-[#555] mb-4">
                    {product.description || "This healing bracelet harnesses the natural energy of gemstones to promote balance and wellness. Each stone carries unique vibrations that support physical, emotional, and spiritual well-being."}
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl">
                  ...
                  <div className="bg-[#fcfaf8] rounded-xl p-6 border border-[#f0ebd9]">
                    <div className="flex items-center gap-2 mb-6">
                      <Sparkles size={18} className="text-[#d0a061]" />
                      <h4 className="font-bold text-[13px] text-[#251622]">Key Benefits</h4>
                    </div>
            ...
            {activeTab === 'How to Use' && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-[13px] text-[#555]">
                <div className="max-w-3xl">
                  <h3 className="text-lg font-serif text-[#251622] mb-4">How to Wear & Use Your Bracelet</h3>
                  <ul className="space-y-3">
                    <li>• Wear on your left wrist to absorb the healing energy</li>
                    <li>• Can be worn daily for continuous benefits</li>
                    <li>• Set an intention when wearing for enhanced results</li>
                    <li>• Remove during sleep if sensitive to energy</li>
                    <li>• Worn close to skin provides more direct energy transfer</li>
                  </ul>
                </div>
              </motion.div>
            )}

            {activeTab === 'Care & Cleansing' && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-[13px] text-[#555]">
                <div className="max-w-3xl">
                  <h3 className="text-lg font-serif text-[#251622] mb-4">Caring for Your Healing Bracelet</h3>
                  <p className="mb-4"><strong>Physical Care:</strong></p>
                  <ul className="space-y-2 mb-6">
                    <li>• Clean gently with a soft cloth and lukewarm water</li>
                    <li>• Avoid harsh chemicals or soaps</li>
                    <li>• Store in a cool, dry place</li>
                    <li>• Keep away from direct sunlight for extended periods</li>
                  </ul>
                  <p className="mb-4"><strong>Energy Cleansing:</strong></p>
                  <ul className="space-y-2">
                    <li>• Place under moonlight overnight monthly</li>
                    <li>• Use sage or incense smoke to cleanse</li>
                    <li>• Bury in salt for deep energetic reset</li>
                    <li>• Recharge intention regularly for best results</li>
                  </ul>
                </div>
              </motion.div>
            )}
          ...
          <div className="mt-8 md:mt-16 pt-8 border-t border-gray-200">
            <div className="flex items-end justify-between mb-8 px-4 md:px-0">
              <div>
                ...
                <h3 className="text-2xl md:text-3xl font-serif text-[#251622]">Other Healing Bracelets</h3>
              </div>
              <Link to="/bracelets" className="text-[11px] font-bold uppercase tracking-wider text-[#4a154b] hover:text-[#d0a061] transition-colors flex items-center gap-1">
